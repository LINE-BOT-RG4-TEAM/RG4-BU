<?php
    ...
    $fetch_quantity_by_year = "
        SELECT  DATE_FORMAT(DATE_ADD(`HISTORY`, INTERVAL 1 YEAR), '%Y-%m') AS `year-month`
                , COUNT(DATE_FORMAT(DATE_ADD(`HISTORY`, INTERVAL 1 YEAR),'%Y-%m')) AS `quantity_jobs`
        FROM history
    ...
    $jobs_by_year_month_results = $conn->query($fetch_quantity_by_year);

    if($jobs_by_year_month_results->num_rows <= 0){
        echo json_encode(array());
        http_response_code(404);
        exit(1);
    }

    $all_jobs = $jobs_by_year_month_results->fetch_all(MYSQLI_ASSOC);
