<div class="row">
<input type="hidden" name="hidden_cate_id" id="hidden_cate_id" value="<?=$_GET['cate_id']?>" />
  <h4 class="p-3 font-weight-bold pt-3 text-center text-primary" id="content_header">
  <!-- <h6 class="p-3 font-weight-bold pt-3 rounded-pill bg-primary text-white" id="content_header"> -->
  </h4>
</div>
<div class="row">
    <div class="col-sm-12 col-md-12 col-lg-12 mt-3">
        <div class="panel panel-primary">
            <div class="p-2 panel-body border text-justify" id="content_body" style="text-indent: 25px;">
            
            </div>
        </div>
    </div>
</div>
<!-- <div class="dropdown-divider"></div> -->
<div class="row" id="card-area">

</div>
<!---->
